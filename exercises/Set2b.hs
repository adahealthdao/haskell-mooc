...
binomial _ 0 = 1
binomial 0 k = 0 --k has to be > 0 to reach this case
...
oddFactorial 0 = 1
oddFactorial n = 
    if odd n then  n * oddFactorial (n - 1)
...
myGcd 0 b = b
myGcd a 0 = a
myGcd a b = if a < b then myGcd a (b - a)
            else myGcd (a - b) b
...
leftpad str n = 
    if length str < n then leftpad (" " ++ str) n
    else str
...
countdown n = countdown' "Ready! " n
  where countdown' str 0 = str ++ "Liftoff!"
...
smallestDivisor n = if even n then 2 else findDivisor 2 n
    
findDivisor :: Integer -> Integer -> Integer
findDivisor k n = if n `mod` k == 0 then k
  else findDivisor (k + 1) n
------------------------------------------------------------------------------
