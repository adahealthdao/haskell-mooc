...
palindromify s = palindromer (isPalindrome s) s

palindromer :: Bool -> String -> String
palindromer _ "" = ""
palindromer True s = s 
palindromer False s = 
    let ss = tail (init s)
    in palindromer (isPalindrome ss) ss
...
safeIndex xs i = 
    if i < length xs && i >= 0 then Just (xs !! i)
    else Nothing
